puts File.open("in.txt") { |file, ed = [0, 0], threadc = 8|
  "#{(out = file.map { |line| (ed = [[ed[0], (a = line.split(",").map(&:to_i))[1]].max, [ed[1], a[0]].max]) && a })[threadc.times.map { |tid, size = (ed[0] + 1)|
    Thread.new { |board = []|
      ...
      ((last = 0)..(out.length + threadc) / threadc).each { |r, queue = [[0, 0, 0]], min = nil, seen = {}, y, x, steps|
        next if (r = r * threadc + tid) >= out.length
        (last..r).each { |r1| board[out[r1][1]][out[r1][0]] = 1 }
        ((next if seen[(k = y * size + x)]) || (seen[k] = steps)
        board[y][x] = 3
        break (min = steps) if ed == [y, x]
        [[y + 1, x], [y - 1, x], [y, x + 1], [y, x - 1]].each { |yy, xx|
          queue.insert((idx = queue.bsearch_index { |b| steps + 1 > b[2] }) ? idx : -1, [yy, xx, steps + 1]) unless yy > ed[1] or xx > ed[0] or yy < 0 or xx < 0 or board[yy][xx] == 1
